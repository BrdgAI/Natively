  ...
  return (
    <div className="interview-surface-primary pointer-events-auto flex min-h-0 min-w-0 flex-col rounded-[10px] px-1.5 py-1.5">
      <div className="mb-1 flex items-center justify-between gap-2 px-0.5">
        <div className="flex items-center gap-1.5 text-[12px] font-bold text-[#f0e4d2] interview-text-heading">
          <FileCode2 size={12} />
          <span>{codePane?.title || 'Code'}</span>
        </div>
        {codePane && (
          <div className="flex items-center gap-1.5">
            <span className="rounded-[4px] border border-[rgba(255,220,180,0.12)] bg-[rgba(255,255,255,0.06)] px-1.5 py-0.5 text-[10px] font-semibold text-[#e6d4b8]">
      ...
      {codePane ? (
        <>
          <div className="interview-code-surface min-h-0 flex-1 overflow-auto px-0 py-0.5">
            {codePane.content.split('\n').map((line, index) => (
              <div
                key={`${index}-${line}`}
                className={`grid grid-cols-[38px_minmax(0,1fr)] gap-1.5 px-1 py-[1px] ${lineClassName(codePane.kind, line)}`}
              >
                <span className="select-none text-right text-[11px] font-mono font-semibold text-[#8e8276] leading-[1.5]">
                  {index + 1}
                </span>
                <span
                  className="min-w-0 whitespace-pre-wrap break-words text-[13px] font-mono font-semibold leading-[1.55] text-[#eadfcd]"
          ...
          {codePane.notes.length > 0 && (
            <div className="mt-1 space-y-0.5 rounded-[7px] border border-[rgba(255,220,180,0.10)] bg-[rgba(255,255,255,0.04)] px-2 py-1.5">
              {codePane.notes.map((line, index) => (
                <div key={`${line}-${index}`} className="text-[11.5px] font-semibold leading-[1.45] text-[#ddcdb5] interview-text">
                  {line}
                </div>
              ))}
            </div>
        ...
        <div className="flex flex-1 items-center justify-center rounded-[7px] border border-dashed border-[rgba(255,228,200,0.14)] px-3 py-5 text-center text-[11.5px] leading-[1.45] text-[#d5c4ae] interview-text">
          {emptyMessage}
        </div>
      )}
    </div>
